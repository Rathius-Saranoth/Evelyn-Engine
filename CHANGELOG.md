...
`[[Ricky|Ricky]]` (373) — the shape identity parameterization leaves behind when it rewrites
...
inside every wikilink it captured: `[[Ricky|Ricky]]` became `[[Ricky Ricky]]` and
...
- **Forward Pipeline Hardening (`Evelyn/tools/fact_extractor.py`, `fact_deduplicator.py`, `fact_splitter.py`, `ingest_obsidian_knowledge.py`)**:
  ...
  - Updated few-shot extraction examples covering first-person user facts, assistant reactions, and third-party actors (Fox, Schyler).

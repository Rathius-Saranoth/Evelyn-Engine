...
def parse_replika_archive(file_path: str) -> list[dict]:
    ...
    for _line_no, raw_line in enumerate(lines, start=1):
        ...
        # Speaker tag match: [Evelyn - 03/12/2025] or [Alex - 03/12/2025]
        msg_match = re.match(r"^\[(Evelyn|Alex)\s*-\s*(\d{1,2}/\d{1,2}/\d{4})\]\s*(.*)$", line)
        if msg_match:
...
def parse_takeout_html(html_path: str) -> list[dict]:
    ...
    evelyn_keywords = [
        "evelyn", "alex", "darling", "my love", "good morning", "gnite",
        "goodnight", "journal", "dream", "fox", "skyler", "jordan",

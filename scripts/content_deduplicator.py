...
Compares all staged knowledge files against:
  ...
  2. The Authoritative EHR Master Notes (Alex/Medical/)
...
def run_deduplication():
    ...
    for file_id, meta in manifest.get("items", {}).items():
        ...
        if meta.get("status") == "EHR_RAW" or "medical record" in rel_path.lower():
            ...
            meta["triage_reason"] = "Official Provider EHR Export (Synthesized into Alex/Medical/ master notes)"
...
def generate_removal_checklist(manifest: dict, results: dict):
    ...
    lines = [
        ...
        "> [!NOTE]",
        "> The entire `Medical Record` folder from your healthcare provider has been safely preserved in local server storage (`data/medical_records/`) and synthesized into authoritative master notes in `Alex/Medical/`.",
        "",

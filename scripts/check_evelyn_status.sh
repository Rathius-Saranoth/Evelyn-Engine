...
# 5. Check Remote FLUX Image Server on alex-pc
IMAGE_URL="http://alex-pc.tail0e161b.ts.net:5055/health"
if curl -s --max-time 3 "$IMAGE_URL" | grep -q "ok" > /dev/null 2>&1; then
    echo -e "\033[0;32m✅ [Remote Image Host] FLUX.1 server reachable on alex-pc:5055.\033[0m"
else
    echo -e "\033[0;33m⚠️  [Remote Image Host] FLUX.1 server on alex-pc:5055 unreachable or offline.\033[0m"
fi

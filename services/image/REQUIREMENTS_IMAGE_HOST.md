# Alex-PC Standalone Image Generation Host Requirements & Restoration Guide

This document defines the system requirements, environment configuration, dependency installation, model setup, and service restoration procedures for the standalone **FLUX.1 [schnell] NF4 Image Generation Server** running on `alex-pc` (`alex-pc.tail0e161b.ts.net:5055`).

Use this guide if the environment on `alex-pc` is wiped, cleaned up, or needs to be reconfigured from scratch.
...
| :--- | :--- | :--- |
| **OS** | Windows 10/11 64-bit (or Linux x86_64) | Tested on Windows 11 host (`alex-pc`) |
...
| **Tailscale** | Tailscale network client logged into tailnet | Enables `alex-pc.tail0e161b.ts.net` routing |
...
On `alex-pc`, place or clone the repository to your chosen project path (e.g. `C:\evelyn` or `%USERPROFILE%\evelyn`).
...
Open PowerShell (or Command Prompt) on `alex-pc` and navigate to the project directory:
...
1. **Tailscale Binding**:
   - Ensure Tailscale is running and `alex-pc` is connected to the `tail0e161b.ts.net` network.
   - Verify `alex-pc` IP using `tailscale ip -4` (e.g. `100.127.160.114`).
...
1. **Health Check Endpoint**:
   ```bash
   curl http://alex-pc.tail0e161b.ts.net:5055/health
   ```
   *Expected Response:*
   ```json
   {
...
2. **Image Generation Test Endpoint**:
   ```bash
   curl -X POST http://alex-pc.tail0e161b.ts.net:5055/generate \

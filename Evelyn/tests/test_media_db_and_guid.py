...
class TestMediaDbAndGuid(unittest.TestCase):
    ...
    def test_update_media_metadata_and_tags(self):
        ...
        success = media_db.update_media_metadata(
            guid=guid,
            description="Biscuit resting by the sunny window.",
            tags=["#Biscuit", "#cat", "#window"],
            taxonomy_domain="Pets/Biscuit",
        )
        self.assertTrue(success)

        retrieved = media_db.get_media_asset(guid)
        self.assertEqual(retrieved["description"], "Biscuit resting by the sunny window.")
        self.assertIn("#Biscuit", retrieved["tags"])
        self.assertEqual(retrieved["taxonomy_domain"], "Pets/Biscuit")

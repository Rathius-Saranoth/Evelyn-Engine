    ...
    return [
        {
            ...
            "observation": "Alex is an analytical thinker who values deep focus.",
        ...
        },
        {
            ...
            "observation": "Jordan visited for the weekend and cooked dinner.",
            "tags": "jordan, social, cooking",
            "date": "2026-08-02",
            "created_at": 1010.0,
            "updated_at": 1010.0,
        },
        ...
        },
        {
            "id": 4,
            "category": f"Cat06-{cfg.SUBJECT_CODE_USER}",
            "observation": "Jordan called to discuss upcoming travel plans.",
            "tags": "jordan, travel",
            "date": "2026-08-04",
            "created_at": 1030.0,
            "updated_at": 1030.0,
...
def test_cluster_entries_by_theme_grouping(sample_user_entries):
    ...
    # Check Relationship Dynamics batch (entries 2 and 4 with tag 'jordan')
    rel_batch = next(b for b in thematic_batches if "Relationship Dynamics" in b["theme_name"])
    assert len(rel_batch["entries"]) == 2
    assert "[Topic / Subject: Jordan]" in rel_batch["evidence_text"]
...
async def test_proofread_document_success():
    ...
    input_body = (
        ...
        "Alex is an analytical thinker. While he navigms complex systems, he preserves his core values.\n\n"
        "## Relationship Dynamics\n\n"
        "He values open communication with Evelyn."
    )
    mock_llm_response = (
        "## Identity & Core Values\n\n"
        "Alex is an analytical thinker. While he navigates complex systems, he preserves his core values.\n\n"
        "## Relationship Dynamics\n\n"
        "He values open communication with Evelyn."
    )
...
async def test_proofread_document_safety_fallback():
    ...
    input_body = (
        "## Identity & Core Values\n\n"
        "Alex is an analytical thinker.\n\n"
        "## Relationship Dynamics\n\n"
        "He values open communication."
    )
    # Severe truncation
    bad_truncated_response = "Alex is an analytical thinker."

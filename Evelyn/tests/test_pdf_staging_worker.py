...
class TestPdfStagingWorker(unittest.TestCase):
    ...
    def test_process_sidecar_only_staging(self):
        ...
        with open(meta_path, "w", encoding="utf-8") as f:
            f.write('{"target_path": "Alex/Medical", "domain": "Medical"}')

        result = staging_worker.process_staging_item(pdf_path, mode="card")
        self.assertEqual(result["status"], "success")

        # Check sidecar created in Alex/Medical
        expected_note = self.vault_root / "Alex" / "Medical" / "Sample Medical Record" / "Sample Medical Record_index.md"

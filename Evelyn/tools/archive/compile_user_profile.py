"""
compile_user_profile.py — Assembles Alex's compiled user profile for Evelyn.

Scans the Context Category Summaries directory for all ``Cat*-R.md`` files
(the "-R" suffix denotes Alex's profile categories) and concatenates their
...
def compile_user_profile():
    ...
    # Find all Cat*-R.md files (User/Alex's summaries)

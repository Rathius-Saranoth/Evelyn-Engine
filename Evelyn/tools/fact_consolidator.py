...
def _build_anchor_prompt(
    ...
    return (
        ...
        "3. FACTUAL ENHANCEMENT: The entries describe the exact same distinct fact about the same subject, and one adds a qualifying detail that refines it (e.g., 'Plays Skyrim' vs 'Plays Skyrim with heavily customized mods'). Do NOT flag entries that merely share a subject but describe different aspects of it.\n\n"
        ...
        "- THE EVENT EXCEPTION: Distinguish between 'States' (persistent, mutable facts such as preferences, traits, or ongoing conditions) and 'Events' (specific, time-bound occurrences such as daily moods, health logs, interactions, or task completions). Sequential events occurring on different dates are distinct historical records \u2014 NOT an evolving state. Do NOT flag them for consolidation.\n"
        "- Do NOT group specific, distinct details into a general category summary.\n"
        "- When confidence is low, do not flag the entry for consolidation.\n\n"
...
async def generate_consolidation_proposal(cluster: dict) -> str | None:
    ...
    prompt = (
        ...
        f"4. THE EVENT EXCEPTION: If the entries represent discrete events, moods, or occurrences tied to different dates (State-Data vs Time-Series Data), they are a historical log \u2014 not a single evolving state. You MUST choose 'keep_both'.\n"
        f"5. If verdict is 'keep_both', set merged_summary to an empty string.\n"
        f"6. NEVER produce a merged_summary that abstracts distinct facts into a vague summary. The output must be as specific and include the same facts as the input."
        f"{cat_ref}\n\n"

...
def _format_messages_for_extraction(messages: list[dict]) -> str:
    ...
    lines = []
    for msg in messages:
        role_label = "Alex" if msg["role"] == "user" else "Evelyn"
        content = msg["content"]
        if len(content) > 600:
            content = content[:597] + "..."
        ts = msg.get("ts")
...
def _build_extraction_prompt(messages: list[dict], cat00: str) -> str:
    ...
    return (
        ...
        "about Alex (the user) or Evelyn (the AI). "
        ...
        "```facts\n"
        "facts:\n"
        "  - subject: Alex          # or Evelyn\n"
        "    category: Cat05-R        # best matching Cat##-E or Cat##-R code\n"
        "    tags: \"kw/alex, kw/habit\" # comma-separated semantic tags starting with kw/\n"
...
def _parse_facts_yaml(raw: str, fallback_date: str) -> list[dict]:
    ...
    for item in facts:
        ...
        if not cat.endswith("-E") and not cat.endswith("-R"):
            suffix = "-R" if subj.lower() == "alex" else "-E"
            cat = cat_base + suffix
...
def _build_procedure_extraction_prompt(messages: list[dict]) -> str:
    ...
    return (
        ...
        "instructions, workflows, rules, or guidelines that Alex (the user) asks Evelyn (the AI) to follow. "

...
Usage:
  ...
  entry_id = memory_db.insert_entry(category='Cat05-U', subject='Alex', ...)
...
def split_entry(source_entry_id: int, new_entries: list[dict]) -> list[int]:
    ...
    default_subject = source.get("subject", getattr(cfg, "USER_NAME", "Alex"))
...
def apply_fact_merge(
    ...
    if not source_entries:
        return insert_entry(
            ...
            subject=getattr(cfg, "USER_NAME", "Alex"),
...
def _extract_keywords(text: str) -> set[str]:
    ...
    stopwords = _STATIC_STOPWORDS | {
        getattr(cfg, "USER_NAME", "Alex").lower(),

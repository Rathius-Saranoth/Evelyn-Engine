...
- [x] **Idle-Time Fact Consolidation**: `fact_consolidator.py` scans all live `Context Entries/Cat##/Cat##-{E,R}/*.md` files during server idle time (30 min threshold). Uses LLM with `think=True` + Cat00 taxonomy injection to detect duplicate/conflicting/superseded facts within each category via a **rotating group scan** (advances `CONSOLIDATION_GROUP_SCAN_LIMIT` categories per run so the full vault cycles across multiple passes). Produces human-readable `CONSOLIDATION_*.md` proposal files in `Pending/` for review. Audits category assignments and flags miscategorized entries. Nothing auto-applied. Mutual exclusion with extractor prevents concurrent Ollama calls. Config: `CONSOLIDATION_*` keys in `evelyn_config.py`. *(Completed 2026-05-04)*

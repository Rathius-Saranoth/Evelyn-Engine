...
date modified: 2026-09-19 09:43:56
---
...
- [x] **Code & Terminal Agency**: Scoped execution tools with security tiers, smart vault auto-resolution, paginated direct document reading with dynamic token scratchpad compaction, interactive Chat UI approval cards, and FastAPI terminal endpoints.
- [x] **Profile Auto-Evolution & Factoid Ledger**: Background memory scanner proposing updates to persona, profile, and directive documents via authoritative factoid ledgers, strict 4-tier precedence hierarchy (`Core -> System -> Assistant -> User`), adaptive deduplication, and section-aware item-level diff review in DevUI.
...
- [x] **Native Multi-Channel Agentic Architecture & Telemetry**: Unified streaming inference loop consuming the model's native reasoning, tool-call, and content channels directly, with structured per-round traces persisted for identical live and restored rendering, a reasoning budget that breaks runaway thought loops, canonical XML telemetry envelopes, direct dense vector RAG, and dynamic tool surfacing.

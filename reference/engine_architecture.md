...
graph TD
    ...
    subgraph SyncMesh [Multi-Device Syncthing Mesh]
        ...
        SyncthingServer["Syncthing Daemon (Alex-PC-WSL:22000)"]
    end
...
* **[[evelyn_server.py]]**: Main FastAPI server codebase. Exposes `/chat` (streaming response), `/regenerate` (response regeneration), and `/edit` (`edit_last_user_message()`) endpoints alongside history, status, and system tool APIs. Implements **adaptive day-bound history loading** (`load_history()`: 100% of today's messages + up to 6 from yesterday, governed by safe token budgeting and turn-integrity pruning against `NUM_CTX`), date-filtered journal isolation, and **dynamic thinking effort** via a multi-tiered architecture: fast pre-classification (`classify_message_effort`), tool-driven effort escalation (`TOOL_THINK_EFFORT`), and manual UI override. Consumes Gemma 4's native multi-channel output directly — reasoning from the `thinking` field, tool calls from `tool_calls`, content passed through verbatim — and emits a **structured per-round reasoning trace** (`round`, `thinking`, `tools` with per-tool status, `budget_tripped`) persisted to `messages.trace_json`, which the UI renders identically live and on reload. A reasoning budget (`THINK_BUDGET_CHARS`) cancels and re-issues a round with thinking disabled when native reasoning runs away, guaranteeing a response. Permanently records resolved effort and resolution source (`think_effort`, `think_source`) in SQLite `message_metrics`. Loop rounds use a configurable token budget (`TOOL_LOOP_NUM_PREDICT`) distinct from the full response budget.
* **[[evelyn_config.py]]**: Single source of truth config file. Controls LLM parameters, on-demand memory thresholds, allowed Tailscale CORS origins, and system path settings. Key agentic & thinking parameters: `THINK` (default response effort: `"medium"`), `THINK_TOOL_LOOP` (tool round effort: `False` / disabled to prevent duplicate pre-drafting reasoning), `THINK_BUDGET_CHARS` (reasoning ceiling per round before the loop re-issues without thinking), `TOOL_LOOP_NUM_PREDICT` (per-round token budget: `1024`), `NUM_PREDICT` (streaming response generation budget: `8192`), `SHOW_TOOL_LOOP_THINKING` (surface intermediate reasoning to UI), `MAX_TOOL_ROUNDS` (loop cap).
...
* **[[ingest_obsidian_knowledge.py]]**: Full-vault full-text memory ingestion engine. Recursively scans `/home/rathius/obsidian_vault` (1,202 files), enforcing 1,600-character chunking and automatic `rag_priority: high` for core identity files (`Alex - Psychological Blueprint.md`, `Evelyn Narrative Persona.md`, `System Directives.md`, `CE_*.md`, `EX_*.md`).
...
* **[[web_reader.py]]**: Dynamic web scraper and direct link extraction engine. Features Trafilatura integration, desktop client fingerprinting, WAF/Cloudflare challenge diagnostics with recovery blocks, synchronous extraction, and adaptive chunking.
...
* **[[evelyn_tools.py]]**: Definitive tool definitions library containing 27 active model tools (`write_journal_entry`, `generate_image`, `web_search`, `read_url`, `start_research`, `list_research_tasks`, `inspect_research_task`, `guide_research`, `check_new_research`, `search_history`, `create_calendar_event`, `delete_calendar_event`, `sync_google_calendar`, `create_task`, `complete_task`, `delete_task`, `list_tasks`, `sync_google_tasks`, `get_agenda`, `manage_vault_list`, `run_command`, `read_file`, `write_file`, `search_reference_library`, `search_vault_notes`, `read_document_scratchpad`, `search_available_tools`). Features paged line reading in `read_file`, deterministic algorithmic document chunking in `read_document_scratchpad`, mid-thinking dynamic tool discovery in `search_available_tools`, defensive parameter sanitization for Gemma 4 12B, conversational URL intercept routing in `web_search`, query sanitization, and in-memory TTL caching. Contains `_log_deprecation()` which logs yellow console warnings and appends full tracebacks to `data/deprecation_warnings.log` whenever deprecated static read tools (`search_vault`, `recall_specific_memory`, `read_journal`) are called out-of-band. Contains `_is_research_engine_running()` — the OS-level PID-based guard against duplicate research subprocess spawning.
...
| `task_manager.acquire_next_idle_task()` | Pop and return the next task from the front of the FIFO queue |
| `task_manager.acquire_next_runnable_task(idle_seconds)` | Pop and return the oldest runnable task matching its cognitive schedule tier |
| `task_manager.get_task_schedule(name)` | Classify task into its cognitive tier (`REFLEX`, `DIURNAL`, `NOCTURNAL`) |
...
**How it works:** `task_manager` coordinates mutual exclusion, fair scheduling, and cooperative batching across all background processes. It classifies tasks into **Cognitive Tiers** (`REFLEX` 24/7 reactive housekeeping, `NOCTURNAL` overnight Digital Dreaming, `DIURNAL` daytime research) and maintains a persistent FIFO idle queue (`data/evelyn_task_queue.json`). Long backlogs drain continuously without starving peer tasks or causing Head-of-Line blocking. After each batch, tools commit progress and call `should_yield()`. When user chats begin, `set_chat_preemption(True)` instantly preempts active tasks and re-enqueues them to the tail of the idle queue, ensuring fast reflex tasks run first once the 5-minute quiet threshold is satisfied.

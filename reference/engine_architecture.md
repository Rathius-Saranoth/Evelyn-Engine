...
graph TD
    ...
    subgraph LocalAI [Inference & Voice Services]
        ...
        subgraph RemoteHost [Remote Tailscale Host: alex-pc]
            Image["FLUX.1 Schnell Image Host<br>(http://alex-pc.tail0e161b.ts.net:5055)"]
        end
    end
...
* **[[image_server.py]]**: FLUX.1 [schnell] server running off-node on `alex-pc` over Tailscale (`http://alex-pc.tail0e161b.ts.net:5055`) to leverage workstation GPU resources.

...
cat >> ~/.ssh/config << 'EOF'
Host evelyn
    HostName alex-pc.tail0e161b.ts.net
    User alex
...
| Symptom | Cause | Fix |
|---|---|---|
| `nc -zv ... 22` gives "unknown option" | Termux netcat doesn't support `-v` | Use `ssh alex@alex-pc...` directly instead |
...
1. **Is the user in the Administrators group?**
   ```powershell
   net localgroup Administrators | findstr alex
   ```

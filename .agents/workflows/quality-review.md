...
> Navigation: [[AGENTS.md]] · [[engine_architecture.md]] · [[README.md]]

Run this after completing any significant feature, refactor, or database migration before closing the task.
...
- [ ] **Targeted Pytest Suite** — Run `PYTHONPATH=. /home/rathius/evelyn/venv/bin/pytest Evelyn/tests/test_<subsystem>.py` for the modified subsystems and confirm all tests pass. (⚠️ NEVER run an unbounded `pytest Evelyn/tests` across the full repo in WSL2; heavy ML dependencies and vector stores will exhaust VM memory and lock up WSL2).

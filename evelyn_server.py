...
def get_research_context() -> str:
    ...
    if stalled_tasks:
        ...
        lines.append("You should mention these to Alex so he can provide guidance, or you can use the 'guide_research' tool to adjust the search terms yourself.")
    ...
    if unnotified_count > 0:
        lines.append(f"\n(Context note: {unnotified_count} newly completed deep research task(s) are ready. You may call 'check_new_research' if relevant to Alex's prompt.)")
...
def get_upcoming_agenda_prompt_context() -> str:
    ...
    try:
        ...
        lines = []
        if events:
            lines.append(f"(Context note: Alex has {len(events)} upcoming calendar event(s) in the next 24 hours. You may call 'get_agenda' if Alex asks about his schedule.)")
...
if __name__ == "__main__":
    ...
    SSL_KEY = "alex-pc.tail0e161b.ts.net.key"
    SSL_CERT = "alex-pc.tail0e161b.ts.net.crt"
    ssl_args = {}
